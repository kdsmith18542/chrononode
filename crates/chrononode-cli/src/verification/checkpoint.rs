...
use chrononode_core::{ChronoBlock, CoreConfig, Result, StoragePointer};
...
impl CheckpointBuilder {
    ...
    pub fn build_checkpoint(
        ...
        let leaves: Vec<MerkleLeaf> = blocks
            .iter()
            .map(|(b, p)| {
                MerkleLeaf::from_block(b, &p.backend, &p.to_string())
            })
            .collect();
        let root = proof::merkle_root(&leaves)
            .ok_or_else(|| chrononode_core::CoreError::Proof("empty checkpoint".to_string()))?;
        Ok(CheckpointResult {
            checkpoint_id: format!("{}-{}-{}", chain_id, start_height, start_height + blocks.len() as u64 - 1),
