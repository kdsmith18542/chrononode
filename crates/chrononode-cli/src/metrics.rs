...
pub fn record_block_served(chain_id: &str) {
    metrics::counter!("chrononode_blocks_served_total", "chain" => chain_id.to_string()).increment(1);
}

pub fn record_proof_verified(valid: bool) {
    metrics::counter!("chrononode_proofs_verified_total", "valid" => valid.to_string()).increment(1);
}

pub fn record_block_archived(chain_id: &str, height: u64) {
    metrics::counter!("chrononode_blocks_archived_total", "chain" => chain_id.to_string()).increment(1);
    metrics::gauge!("chrononode_archive_depth", "chain" => chain_id.to_string()).set(height as f64);
}

pub fn record_ingest_error(chain_id: &str) {
    metrics::counter!("chrononode_ingest_errors_total", "chain" => chain_id.to_string()).increment(1);
}

pub fn record_storage_operation(backend: &str, operation: &str, success: bool, duration: std::time::Duration) {
    metrics::counter!(
...
pub fn record_checkpoint_created(chain_id: &str) {
    metrics::counter!("chrononode_checkpoints_created_total", "chain" => chain_id.to_string()).increment(1);
}
